...
# Размеры пакетов сверяются не с числами, вписанными сюда, а между собой: README называет их
# трижды (вводный пункт «Движок — свой», строка «от … МБ свободного флеша» и таблица вариантов), и разойтись
# они могут молча — как разошлись константы этого стенда, когда README переписали по ассетам
# релиза 1.3.0, а стенд продолжал требовать числа предыдущего выпуска. Источник — таблица:
# именно её человек читает, выбирая вариант.
size_of() {  # ИМЯ_ПАКЕТА → «X–Y КБ» из строки таблицы
    grep -F "(\`$1\`)" "$ROOT/README.md" | sed -n 's/.*| *\([0-9][0-9]*–[0-9][0-9]* КБ\) *|.*/\1/p' | head -1
}
EXT_SZ="$(size_of steer-extended)"
BASE_SZ="$(size_of steer)"
check "таблица называет размер расширенного варианта" "да" "$([ -n "$EXT_SZ" ] && echo да || echo нет)"
check "таблица называет размер базового варианта" "да" "$([ -n "$BASE_SZ" ] && echo да || echo нет)"
check "вводный пункт называет тот же размер расширенного" "да" "$(has README.md "$EXT_SZ в расширенном")"
check "вводный пункт называет тот же размер базового" "да" "$(has README.md "$BASE_SZ в базовом")"
# Строка про флеш даёт одну вилку на оба варианта: от меньшего базового до большего расширенного.
check "вилка «от … МБ» собрана из тех же чисел" "да" \
