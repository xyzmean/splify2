...
# ---- СЛОЙ YouTube: связка, а не только основная -----------------------------------------
#
# ЧТО ЗДЕСЬ СТОРОЖИТСЯ. Прогон измеряет и стратегии Yv — у них свой набор целей (домены
# YouTube) и свой контрольный проход, — но до этой правки ответ никуда не шёл: рейтинг слои
# не берёт (победителя применяют как основную), и человек получал лучшую основную вместе со
# слоем, который был у него раньше. Zapret Manager ставит слой поверх основной своим прогоном,
# то есть в связке; связки не было у нас, и обратка назвала это прямо.
rank_y="$(zp_rank_layer youtube)"
check "в рейтинге слоёв только слои" "2" "$(printf '%s\n' "$rank_y" | grep -c '^')"
check "и основных стратегий там нет" "0" \
    "$(printf '%s\n' "$rank_y" | cut -f6 | grep -vc '^Yv')"
check "лучший слой — тот, что открыл больше" "Yv1" \
    "$(printf '%s\n' "$rank_y" | head -n1 | cut -f6)"

zp_winner_layer youtube
check "победитель слоя есть" "0" "$?"
check "и это Yv1" "Yv1" "$(printf '%s' "$ZP_WINNER_LAYER" | cut -f6)"

# Слой, не открывший больше, чем открывается без обхода вовсе, применять незачем — тот же
# довод, что у основной стратегии.
base youtube 12 12
zp_winner_layer youtube
check "слой не лучше, чем без обхода — отказ" "1" "$?"
check "и сказано про «без обхода вовсе»" "1" \
    "$(printf '%s' "$ZP_NOTE" | grep -c 'без обхода вовсе')"
base youtube 2 12
...
# СВЯЗКА. В значении обязаны оказаться обе части: основная-победитель и лучший слой YouTube.
check "слой YouTube выбран прогоном" "1" "$(zp_yv_get)"
check "и он стоит в значении" "1" "$(grep -c '^#Yv1$' "$ZP_CONF")"
check "основная при этом на месте" "v2" "$(zp_active_global)"
check "и связка названа человеку" "1" \
    "$(printf '%s' "$out" | grep -c 'применена v2 + Yv1')"
# Повторный подбор слой не трогает: он уже лучший, а перезапись равного — риск без выигрыша.
out="$(run_cmd --apply --no-test)"
check "слой уже лучший — остаётся прежним" "1" "$(zp_yv_get)"
# И ГЛАВНОЕ ПРО ОБЛАСТЬ `yv`: основную там менять не на что по построению, и выход на этом
# означал бы, что подбор слоя не делает ничего. Слой сбрасываем на худший — прогон обязан
# вернуть лучший, не трогая основную.
zp_yv_set 2 >/dev/null 2>&1
check "слой сброшен на худший" "2" "$(zp_yv_get)"
out="$(run_cmd --apply --no-test --scope yv)"
check "подбор области yv поставил лучший слой" "1" "$(zp_yv_get)"
check "и основную при этом не тронул" "v2" "$(zp_active_global)"
...
# Покой сторожит ТОЛЬКО расписание: проверка изолирована и трафика человека не касается, а
# нажатая кнопка при пороге в 300 кбит/с на живой сети не срабатывала почти никогда.
out="$(run_cmd --apply)"
check "по кнопке занятость роутера подбор не останавливает" "0" "$(printf '%s' "$out" | grep -c 'занят')"
check "и проверка по кнопке запускается" "1" "$([ -s "$tmp/test.log" ] && echo 1 || echo 0)"
: > "$tmp/test.log"
# Кнопка только что прогнала проверку и оставила отметку tested_at — расписанию она по праву
# мешает (см. ниже). Здесь проверяется покой, поэтому отметку сдвигаем так же, как `at`.
sed "s/^tested_at=.*/tested_at=$(( $(date +%s) - 8 * 86400 ))/" "$ZP_AUTOSEL" > "$ZP_AUTOSEL.x"
mv "$ZP_AUTOSEL.x" "$ZP_AUTOSEL"
...
# ПРОГОН БЫЛ, ПРИМЕНЯТЬ НЕЧЕГО — СЛЕДУЮЩАЯ НОЧЬ МОЛЧИТ. Лучшая (v2) уже стоит, и прогон выше
# кончился отказом «уже применена лучшая». Отметку `at` пишет только применение, и если срок
# считать лишь от неё, роутер, у которого всё хорошо, гоняет получасовую проверку каждую ночь
# — ровно то, от чего ключ «число дней между подборами» должен защищать. Срок считается от
# большего из `at` и `tested_at`, и вторую пишет сам прогон.
check "после прогона записана отметка проверки" "1" \
    "$(zp_autosel_get tested_at 2>/dev/null | grep -c '^[0-9][0-9]*$')"
: > "$tmp/test.log"
run_cmd --scheduled >/dev/null 2>&1
check "прогон был, применять нечего — следующая ночь молчит" "0" \
    "$([ -s "$tmp/test.log" ] && echo 1 || echo 0)"
check "отметка применения при этом не тронута" "1" \
    "$([ "$(zp_autosel_get at)" -lt "$(( $(date +%s) - 7 * 86400 ))" ] && echo 1 || echo 0)"
