...
# Телеметрия: главный вопрос стенда — не «правильно ли сделано», а «не уехало ли лишнее».
# Устроен как ЗАПРЕТ на образце с настоящими по форме секретами плюс канарейка во всех
# источниках, которые читает сборщик.
run telemetrymatch sh "$ROOT/tests/telemetrymatch.sh"
