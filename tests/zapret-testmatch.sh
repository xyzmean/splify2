...
#
# Импорт семейства `Dv` (запуск 65) привёл в каталог 17 блоков, которые фильтруют только
# порты discord.media. Ни одна цель, открываемая curl'ом, через них не ходит, поэтому в
# общем наборе они дали бы ноль удач — и человек прочёл бы честный результат как «17
# стратегий не работают».
#
# Проверяется РАЗЛИЧЕНИЕ, а не просто исключение: `Yv` тоже слой, но у него есть свой набор
# целей и свой контрольный проход, и из «всех» он выпадать НЕ должен. Первая редакция правки
# выбросила слои целиком и покраснела восемью проверками выше — этот раздел стоит, чтобы та
# ошибка не вернулась.
printf '#general1\n--filter-tcp=443\n\n#v1\n--filter-tcp=443\n\n#Yv01\n--filter-tcp=443\n\n#Dv1\n--filter-tcp=2053,2083,2087,2096,8443\n' \
    > "$T/zapret/strategies.txt"
rm -rf "$T/zapret/results.d" "$T/zapret/results.json"
CURL_OK="rutracker.org youtube.com" run --scope all >/dev/null 2>&1
check "прогон с Dv в каталоге завершился" "0" "$?"
RD="$T/zapret/results.json"
check "в сводке три стратегии, а не четыре" "3" "$(jq "$RD" 'str(len(d["results"]))')"
check "Dv в сводку не попал" "no" \
    "$(grep -q '"Dv1"' "$RD" && echo yes || echo no)"
check "а Yv попал" "yes" \
    "$(grep -q '"Yv01"' "$RD" && echo yes || echo no)"
# Своей областью слой по-прежнему проверяем: человек, который хочет числа именно по Dv,
# должен их получить — просто не в общем наборе.
check "область dv принимается" "0" \
    "$(CURL_OK="" run --scope dv >/dev/null 2>&1; echo $?)"
