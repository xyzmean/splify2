...
STRUCTURE = [
    ...
    # Предел длины имени устройства виден человеку в браузере, а не в системном журнале.
    # Ядро принимает 15 значащих символов; имя длиннее обработчик теперь отвергает
    # (DEVICE_NAME_TOO_LONG), но отказ доезжает до человека как «интерфейс не поднялся» в
    # «Состоянии» — то есть на два слоя ниже поля, в которое он это имя вписал. Готовый
    # datatype LuCI `netdevname` проверяет ровно то же: 1..15 символов без ':', '/', '%' и
    # пробелов. Утверждение проверяется до следующего taboption, иначе совпал бы datatype
    # соседнего поля.
    ("имя устройства ограничено пределом ядра",
     r"'device_name'(?:(?!o = s\.taboption)[\s\S])*?o\.datatype = 'netdevname';", True),
]
