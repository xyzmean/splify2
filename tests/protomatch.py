...
CASES = [
    ...
    # отбрасываются молча. Молча отброшенный DNS означает, что человек считает его настроенным.
    ("ключ DNS", GOOD.replace("SNI = www.microsoft.com", "DNS = 1.1.1.1"), "DNS"),
...
def main():
    ...
    if fails:
        ...
        return 1
    print("все %d проверок прошли" % (len(CASES) + len(expect)))
    return 0
