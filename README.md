...
сеть 140.82.121.x), и `codeload` отдаёт ветку архивом, никуда не перенаправляя.
...
`api.github.com` или архивом через `codeload`, и только потом через туннель самого роутера,
если он поднят. Чужие зеркала (`gh-proxy` и родня) не используются нигде: зеркалу
пришлось бы доверить то, что роутер выполнит как правила маршрутизации. Обход через
туннель отключается: `uci set splify2.main.fetch_via_tunnel=0`.
