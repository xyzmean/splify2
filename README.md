...
через туннель: **Диагностика → Откуда качать списки и обновления → Через туннель**. Файл
поедет по своему прямому адресу одним запросом. То же из консоли:
`uci set splify2.main.fetch_via_tunnel=always; uci commit splify2`. Значение `off` запрещает
туннель вовсе, `auto` (по умолчанию) оставляет его последней ступенью.
