...
# ОТВЕЧАЕТ ЛИ DoH НА САМОМ ДЕЛЕ — и почему без этой проверки нельзя.
#
# Включая DoH, https-dns-proxy по ключу `dnsmasq_config_update '*'` переписывает конфигурацию
# dnsmasq, и там оказывается `no-resolv` и ровно два upstream — сами прокси (127.0.0.1:5053 и
# :5054). Резолверы провайдера из resolv.conf.auto после этого НЕ используются вовсе. Значит
# запасного пути у сети нет по построению: недоступен endpoint DoH — не разрешается ни одно
# имя, ни у кого.
#
# А endpointы DoH — ровно то, что режут. То есть отказ приходит именно к тем, кто включил DoH,
# чтобы блокировки обойти.
#
# Замерено на стенде в QEMU с настоящим клиентом за LAN: пока endpointы доступны — 0 запросов
# без ответа из 20 за 2 секунды; стоит их зарезать — 20 из 20, каждый висит около 18 секунд.
# Браузер показывает при этом DNS_PROBE_STARTED и сдаётся гораздо раньше.
#
# И всё это время splify2 говорил, что всё хорошо: diag — ноль отказов, doh_state — installed,
# running, enabled, выбранный провайдер. Процесс прокси действительно жив, он просто не может
# достучаться до своего адреса. Проверка «служба запущена» отвечает не на тот вопрос.
#
# ЧТО ИМЕННО ПРОВЕРЯЕТСЯ: цепочка целиком, тем же путём, каким ходит клиент, — запрос имени у
# резолвера роутера. Спрашивать сам прокси на его порту было бы точнее по адресу и хуже по
# смыслу: между клиентом и прокси стоит dnsmasq, и поломка бывает в нём.
#
# ИМЯ КАЖДЫЙ РАЗ НОВОЕ. Одно и то же dnsmasq отдаст из кеша, и мы померим кеш, а не путь до
# провайдера. Отрицательный ответ (NXDOMAIN) нас устраивает полностью: вопрос стоит «ответили
# ли вообще», а не «что ответили».
doh_probe_name() {
    printf 'p%s-%s.example.com' "$$" "$(date +%s 2>/dev/null || echo 0)"
}

# Проба с пределом ожидания СВОИМИ РУКАМИ: команды `timeout` на роутере нет (проверено на
# OpenWrt 25.12.5 — 127, «нет такой команды»), а busybox nslookup ключа таймаута не понимает и
# сам ждёт около восемнадцати секунд. Поэтому запрос уходит в фон, а мы ждём его заданные
# секунды и снимаем, если не дождались.
doh_probe_once() {
    _dp_n="$(doh_probe_name)"
    nslookup "$_dp_n" 127.0.0.1 >/dev/null 2>&1 &
    _dp_pid=$!
    _dp_i=0
    while [ "$_dp_i" -lt "${DOH_PROBE_WAIT:-4}" ]; do
        kill -0 "$_dp_pid" 2>/dev/null || { wait "$_dp_pid" 2>/dev/null; return 0; }
        sleep 1
        _dp_i=$((_dp_i + 1))
    done
    kill "$_dp_pid" 2>/dev/null
    wait "$_dp_pid" 2>/dev/null
    return 1
}

# Ответ с КЕШЕМ, и кеш здесь обязателен. Приговоры спрашивает не только экран диагностики, но и
# круг опроса страницы — а он ходит постоянно. Проба стоит до четырёх секунд, и без кеша каждый
# круг платил бы ими, причём дороже всего ровно тогда, когда DoH мёртв.
#
# Живёт в /var/run: это tmpfs, флеш не изнашивается, и после перезагрузки ответ честно
# считается заново.
DOH_ALIVE=${DOH_ALIVE:-/var/run/splify2-doh-alive}
DOH_ALIVE_TTL=${DOH_ALIVE_TTL:-30}

doh_answers() {
    # Спрашивать нечего, пока DoH не включён: путь к именам тогда обычный, и наша проба
    # ответила бы «жив» про то, чего нет.
    doh_installed || return 0
    doh_running   || return 0
    doh_enabled   || return 0

    _da_now="$(date +%s 2>/dev/null || echo 0)"
    if [ -s "$DOH_ALIVE" ]; then
        read -r _da_at _da_v < "$DOH_ALIVE" 2>/dev/null
        case "${_da_at:-x}${_da_v:-x}" in
            *[!0-9]*) ;;
            *) [ "$((_da_now - _da_at))" -lt "$DOH_ALIVE_TTL" ] && return "$_da_v" ;;
        esac
    fi
    if doh_probe_once; then _da_r=0; else _da_r=1; fi
    # Запись атомарная: круг опроса и экран диагностики читают этот файл одновременно, и
    # обрубок прочитался бы как мусор — то есть проба пошла бы заново на каждом круге.
    printf '%s %s\n' "$_da_now" "$_da_r" > "$DOH_ALIVE.new" 2>/dev/null &&
        mv "$DOH_ALIVE.new" "$DOH_ALIVE" 2>/dev/null || rm -f "$DOH_ALIVE.new"
    return "$_da_r"
}
