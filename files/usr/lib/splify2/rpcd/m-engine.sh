...
# srs-read, выход kind=zapret и reload_zapret, dev-id для отчёта о работе, схема спеки 2),
...
# «набор понят, но списком не выразим», а отчёт о работе не собирался вовсе.
#
# 1.5.3 — движок, выпущенный вместе с 26.9 Andromeda. Первый выпуск с нужными умениями —
# 1.5.0, но между ним и 1.5.3 подписка чинилась трижды (cookie панели, предел длины ссылки,
# набивка xhttp), и промежуточных релизов не было: назвать меньшее число значило бы одобрить
# движок, которого ни у кого нет.
STEER_MIN_VERSION=1.5.3
