...
backup_check_spec() {  # ФАЙЛ
    ...
    # ВЫХОД канала — строгой проверкой: это имя уезжает в имена наборов nftables и в
    # командные строки движка.
    for v in $(jsonfilter -i "$1" -e '@.channels[*].out' 2>/dev/null); do
        backup_safe_word "$v" || { echo "выход «$v» в канале — только латиница, цифры, точка, двоеточие, дефис"; return 1; }
    done
    # ИМЯ канала — ярлыком: оно не уезжает никуда, кроме экрана и сообщений движка, а
    # строгая проверка отбивала «правило1» и «Заблокированное в РФ», то есть ровно те имена,
    # которые интерфейс даёт сам. См. backup_safe_label.
    for v in $(jsonfilter -i "$1" -e '@.channels[*].name' 2>/dev/null); do
        backup_safe_label "$v" || { echo "имя канала «$v»: кавычки, доллар и разделители команд в имени недопустимы"; return 1; }
    done
...
case "$2" in
    ...
    backup_get)
        ...
        if [ "$off" = 0 ]; then
            ...
            # ПЕРЕСОЗДАЁТСЯ и проверяется: umask действует только при создании, а чужой
            # процесс, создавший файл заранее с правами 0644, читал бы архив весь срок
            # выгрузки — fs.protected_regular на роутере выключен.
            rm -f "$BACKUP_OUT"
    ...
    backup_put)
        ...
        # mktemp -d: имя с PID предсказуемо, и чужой каталог на этом месте означал бы разделы
        # архива (аргументы nfqws, спека, ключи туннеля) в руках подложившего между проверкой и
        # копированием.
        D="$(mktemp -d /tmp/splify2-restore.XXXXXX)" || fail "не удалось развернуть архив"
