...
case "$2" in
    ...
    zapret_remove)
        ...
        # СНЯТЬ ОБХОД, НА КОТОРЫЙ ССЫЛАЕТСЯ СПЕКА, НЕЛЬЗЯ МОЛЧА — ровно тот же довод, по
        # которому нельзя молча удалить подписку, занятую выходом (sub_used_by в m-sub.sh).
        #
        # Что происходит, если снять. Выход kind=zapret остаётся в спеке, движок поднимает
        # его при следующем применении, наши правила в таблице splify2_zm продолжают уводить
        # помеченный трафик в очередь nfqueue — а обработчика этой очереди больше нет. Пакеты
        # в очередь без читателя не идут никуда: они просто пропадают. Снаружи это выглядит
        # так, что «после удаления обхода отвалилось всё», причём отвалилось ровно то, что
        # человек через этот выход и пустил, — а на роутере, где выхода kind=zapret нет,
        # ничего не происходит и повторить жалобу нечем.
        #
        # То же самое относится к on_fail=zapret: это запасной путь выхода, и он ведёт туда
        # же. Отказ, а не предупреждение: предупреждение здесь означает «мы знали и всё равно
        # сломали», а исправить спеку — минутное дело и делается на соседнем экране.
        if [ -s "$SPEC" ]; then
            _zr_k="$(jsonfilter -i "$SPEC" -e '@.outputs[*].kind' 2>/dev/null |
                     grep -Fx -c -- zapret 2>/dev/null)"
            _zr_f="$(jsonfilter -i "$SPEC" -e '@.outputs[*].on_fail' 2>/dev/null |
                     grep -Fx -c -- zapret 2>/dev/null)"
            case "${_zr_k:-}" in ''|*[!0-9]*) _zr_k=0 ;; esac
            case "${_zr_f:-}" in ''|*[!0-9]*) _zr_f=0 ;; esac
            if [ "$_zr_k" -gt 0 ]; then
                fail "обход занят: выходов kind=zapret — $_zr_k. Уберите их из каналов, потом снимайте пакет"
            fi
            if [ "$_zr_f" -gt 0 ]; then
                fail "обход занят: у $_zr_f выхода(ов) запасной путь on_fail=zapret. Смените его, потом снимайте пакет"
            fi
        fi
