...
describe('автоподбор стратегии обхода', () => {
    ...
    it('отложенный подбор назван с причиной, а не молчит', async () => {
        // Ночной подбор вышел, не начав: через роутер шёл трафик. Прежде страница показывала
        // прежний приговор и ничего больше (владелец: «подбор вообще ничего не отдаёт»).
        // Причина приезжает из файла хода подбора. По кнопке подбор не откладывается вовсе.
        mockAll({ state: 'skipped', state_note: 'идёт трафик' } as never)
        render(<Zapret />)
        await waitFor(() => expect(screen.getByText(/Подбор по расписанию отложен: идёт трафик/)).toBeInTheDocument())
    })
