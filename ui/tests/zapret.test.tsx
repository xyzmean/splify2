...
describe('вкладка Zapret', () => {
    ...
    it('выход обхода заводится здесь и уходит в ЧЕРНОВИК, а не применяется сам', async () => {
        ...
        // УМОЛЧАНИЕ У ВЫХОДА ОБХОДА — «напрямую», И ЭТО НЕ ТО ЖЕ САМОЕ, ЧТО У ТУННЕЛЯ.
        //
        // Здесь стояло 'drop' с доводом «канал заводят ради обхода, и молча вернуть трафик на
        // открытый путь значит нарушить единственное обещание выхода». Довод верен для
        // выхода-туннеля: там обход правила — это утечка мимо VPN, и запрет дешевле утечки.
        // У обхода DPI утечки нет вовсе: трафик и так шёл в открытый интернет, и вопрос стоит
        // иначе — «провайдер, может быть, пропустит» против «ядро точно уронит».
        //
        // Движок выражает on_fail отсутствием `bypass` у `queue num` (steer.c): при 'drop' и
        // пустой очереди пакет РОНЯЕТСЯ. Значит любая осечка nfqws — не поднялся, упал,
        // перезапускается после «Применить» — превращалась в «то, что в правиле, не
        // открывается вообще». Выбрать другое значение человеку негде: радиокнопка on_fail
        // живёт только у выходов VPN.
        expect(next.outputs.yt2.on_fail).toBe('direct')
