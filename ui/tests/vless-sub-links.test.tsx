...
describe('устройство в панели подписки', () => {
    it('идентификатор роутера показан рядом с подпиской', async () => {
        ...
        //
        // Проверяется ИМЕННО ЭТО ОБЕЩАНИЕ, а не фраза, которой оно высказано. Стенд уже
        // дважды падал на формулировке: сперва ждал «MAC порта» — слова, убранного с экрана
        // нарочно (fe1d806, «состояние и действие, а не причины наших решений»), — потом
        // ждал редакцию, которая в рабочем дереве была, а в ветке ещё нет. Обещание при этом
        // не менялось ни разу, поэтому образец берёт только его: слово MAC и «не
        // отправляется» в одном предложении.
        expect(screen.getByText(/MAC[^.]*не отправляется/)).toBeInTheDocument()
    })
