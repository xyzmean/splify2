...
// Теперь она во вкладке движка, рядом с остальным, что не про маршрутизацию (движок и
// самообновление). Прежний довод при этом выполняется: вкладка доступна всегда, в том
// числе когда настройка ещё не работает и переносить её надо на новый роутер.
describe('где живёт карточка архива', () => {
    it('во вкладке движка, рядом с движком и самообновлением', async () => {
        const { default: LogsTab } = await import('@/components/tabs/LogsTab')
        const { live } = await import('./fixtures')
        render(<LogsTab live={live()} />)
