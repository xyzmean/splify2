...
const EXPECTED_JS = [
  ...
  // Разделы пульта, загружаемые по требованию. Главная здесь НЕ ленивая и своего куска не
  // имеет намеренно: это первый экран, и отдельный запрос за ним означал бы пустое место в
  // момент, когда человек только открыл страницу.
  'splify-RulesTab.js', 'splify-Vpn.js', 'splify-Doh.js', 'splify-Zapret.js',
  'splify-Settings.js',
  // Панели, которые читают ДВА раздела сразу. Своим куском они стали не по нашему выбору:
  // модуль, который импортируют два ленивых куска, rollup выносит в третий. Имена
  // перечислены здесь, потому что build.sh проставляет в каждую внутреннюю ссылку версию —
  // кусок вне этого перечня уехал бы на роутер без пина и однажды достался бы браузеру из
  // старого кеша.
  //
  // ВНУТРИ ленивого раздела ленивых импортов больше НЕТ: вложенный Suspense под preact не
