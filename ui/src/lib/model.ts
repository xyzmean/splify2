...
export interface Output {
     ...
     *  подписки. Не больше шестнадцати номеров; повтор и отрицательный номер — отказ
     *  загрузки. */
    nodes?: number[]
    ...
    opts_file?: string
    /** ЧАСТЬ СОСТАВНОГО ВЫХОДА: имя выхода-пула, которому принадлежит этот выход.
     *
     *  Пул из локаций двух подписок и своего туннеля движок выражает одной формой — выход
     *  `kind: interface`, в `devices` которого названы устройства выходов `kind: vless`
     *  (контракт steer, §выходы: «так и собирается пул разнородных туннелей»). То есть локации
     *  ОДНОЙ подписки внутри пула — это отдельный выход `kind: vless` со своим устройством и
     *  своим списком `nodes`. Такие выходы заводит редактор пула сам, и человеку они не
     *  показываются ни в списке выходов, ни среди целей правила: для него они — строки внутри
     *  одного выхода, а не три выхода. Поле и есть признак «служебный, принадлежит пулу».
     *
     *  ДЛЯ ДВИЖКА ЭТО ПРОСТО ЕЩЁ ОДИН КЛЮЧ, И ОН ЕГО ПРОПУСКАЕТ (совместимость вперёд внутри
...
export interface Channel {
    ...
    enabled?: boolean
    /** Кому: подсети, адреса или MAC-адреса. Пусто — `from_default` спеки.
     *
     *  MAC надёжнее адреса там, где правило про КОНКРЕТНОЕ устройство: адрес по DHCP
     *  сменится, MAC — нет. Смешивать адреса и MAC в одном списке нельзя, движок такую
     *  спеку отвергает: правило «кому» в nft одно, а «или» внутри правила там не бывает. */
    from?: string[]
    /** Правило на ОДНО устройство — старше любого глобального, и старше по построению.
     *
     *  СХЕМА 2. Приоритет в цепочке — это порядок совпадения: правило заканчивается
     *  `return`, поэтому побеждает первое совпавшее. Пока порядок читался только со спеки,
     *  правило «этот телефон не маршрутизируем» работало ровно до тех пор, пока человек
     *  держал его выше глобальных: добавил новое глобальное первым — и исключение
     *  перестало действовать, молча. С этим признаком компилятор проходит спеку дважды и
     *  кладёт устройства первыми, как бы они ни лежали в списке.
     *
     *  Требования: непустой `from` и только одиночные хозяева (адрес, адрес/32 или MAC).
     *  Подсеть движок отвергает — приоритет достался бы не устройству, а всем в ней.
     *
     *  `any` у такого правила законен без `allow_all`: общий запрет заведён против описки
     *  «весь трафик в туннель», у которой цена — все клиенты без интернета; здесь цена
     *  ошибки — один хозяин, и он же её заметит. Так и записываются две вещи, которых
     *  иначе не выразить: «весь трафик этого телефона в туннель» и «этот ноутбук не
     *  маршрутизируем вовсе». */
    scope?: 'device' | 'global'
...
export interface ClientNet {
    ...
    wan: boolean
    subnets?: string[]
    /** Годится ли устройство в ИСТОЧНИКИ трафика клиентов.
     *
     *  В /sys лежат и наши собственные исходящие туннели — устройство выхода kind=vless,
     *  wg-клиент до провайдера, туннель xsteer, — а от туннеля, через который к нам
     *  ПРИХОДЯТ чужие устройства (сервер wireguard, Tailscale, ZeroTier), они там
     *  неотличимы. Различает их бэкенд, и приговор приезжает сюда готовым: повторять его
     *  разбор на экране значило бы завести второе место, знающее про маршрут по умолчанию
     *  и пиров wireguard, и первое же расхождение было бы молчаливым.
     *
     *  Поля НЕТ — объект старее приговора; это не то же самое, что `false`, и экран обязан
     *  различать: иначе на старом объекте серым стал бы весь список сразу. */
    usable?: boolean
    /** Почему не годится — строкой, как её прочитает человек. Пустая у годного. */
    why?: string
}
 ...
 *  ключей.
 *
 *  Оба написания сразу движок с I-295 не принимает вовсе: такую спеку он отвергает целиком
 *  (spec.c:945, :961), и канал не загружается. Интерфейс сам её не пишет, а встретив, сводит к
 *  одной форме — берёт ключ, стоящий в документе ПОЗЖЕ (так решал движок до I-295), — и при
 *  первом сохранении файл снова становится загружаемым. Прежде этот абзац говорил «решается
 *  так же, как у движка», и после I-295 это перестало быть правдой (I-336). */
