...
export const rpc = {
    ...
    /** Согласие на телеметрию — ТРИ состояния, а не два (docs/TELEMETRY.md).
     *
     *  `consent` отдельно от `on` не по недосмотру: «не спрашивали» даёт интерфейсу право
     *  предложить один раз, «отказался» — не даёт никогда, и одним булевым полем их не
     *  различить. Свернув их в `on`, интерфейс показывал бы предложение человеку, который
     *  уже сказал «нет», при каждом открытии страницы.
     *
