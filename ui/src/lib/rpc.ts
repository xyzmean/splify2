...
export const rpc = {
    ...
    subList: declare<{
        subs: {
            ...
            link?: string
            quota?: SubQuota
            /** Через сколько минут роутер обновляет подписку сам. 0 — не обновляет. */
            auto?: number
     ...
     *
     *  У КАЖДОЙ ПОДПИСКИ СВОЙ интервал: у одной панели узлы меняются раз в сутки, у другой —
     *  при каждой смене сервера. Пределы (полчаса … трое суток) проверяет бэкенд, а не только
     *  этот список: метод зовут и мимо страницы. */
    subAuto: declare<{ ok: boolean; error?: string; name?: string; auto?: number }>('sub_auto', [
        'name',
        'minutes',
    ]),

    /** Обновить подписку по сохранённой ссылке. Этим методом ходит расписание; кнопка
     *  «Обновить» на странице работает через subSet, у которого на входе ссылка.
     *
     *  `changed` — изменились ли узлы. Туннель перечитывает их только тогда: подписку
     *  опрашивают по часам, а узлы у панели меняются редко. */
    subRefresh: declare<{
        ok: boolean
        error?: string
        name?: string
    ...
    /** Согласие на телеметрию (docs/TELEMETRY.md). Телеметрия включена ПО УМОЛЧАНИЮ:
     *  согласие подразумевается фактом использования splify2, а отказ — единственное явное
     *  действие, и делается он в «Настройки → О ПО».
     *
     *  ДВА ПОЛЯ ОТВЕЧАЮТ НА РАЗНЫЕ ВОПРОСЫ, и в этом весь смысл их пары. `consent` — три
     *  состояния: человек не трогал переключатель, включил сам, отказался; они нужны, чтобы
     *  подписать один и тот же включённый переключатель по-разному. `on` — уедет ли пакет, и
     *  считает это РОУТЕР одним решением на весь продукт (tm_allowed). Пересчитать его здесь
     *  по `consent` значило бы завести второе место, где живёт то же правило, — и однажды
     *  экран показывал бы не то, что происходит на роутере.
     *
     *  `id` пуст, пока идентификатор не посчитан: считать его ради ответа на опрос страницы
     *  нельзя — это 600 000 проходов PBKDF2. */
    telemetryState: declare<{
        consent?: string
        on?: boolean
        id?: string
        last_at?: number
    ...
    /** Записать согласие. Отказ на роутере ещё и УБИРАЕТ посчитанный идентификатор. */
    telemetrySet: declare<{ ok: boolean; consent?: string; error?: string }>(
        'telemetry_set', ['on']),

    /** ТЕ ЖЕ БАЙТЫ, которые уедут: сборщик пакета один, и предпросмотр зовёт его же.
     *
     *  Ответ — САМ ПАКЕТ, а не обёртка вокруг него, поэтому тип здесь нарочно нетипизирован:
     *  описать схему полей означало бы завести второй её экземпляр в интерфейсе, а карточка
     *  обязана показать то, что приехало, включая поля, о которых она не знает. Отказ
     *  приезжает общей формой объекта: `{ ok: false, error }`. */
    telemetryPreview: declare<Record<string, unknown>>('telemetry_preview'),
     ...
     *  попробовавший чужой, должен уметь вернуться, не вспоминая адрес. */
    listsSource: declare<{ ok: boolean; url: string; default_url: string; default: boolean; error?: string }>(
        'lists_source',
    ),
