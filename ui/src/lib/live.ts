    ...
    useEffect(() => {
        ...
        const load = async (force?: 'diag') => {
            ...
            try {
                ...
                setStatus(r.status)
                setError(null)
                /* Ответ движка ЗАПОМНЕННЫЙ — значит показанное всё ещё прошлое, и свежий круг
                 * уже выпущен ниже. Экран говорит «Обновление…» ровно до его ответа. */
            ...
            /* Свежее — СРАЗУ за памятью, а не через пять секунд. Просить память имеет смысл
             * только затем, чтобы что-то нарисовать немедленно; оставить её на экране до
             * следующего тика значило бы показывать прошлое пять секунд там, где до сих
             * пор показывалось «Загрузка…». Вызов вне finally: inflight к этому моменту уже
             * снят, иначе второй круг отбросил бы сам себя. */
            if (fast && cachedNow && !stop) void load()
        }
        void load()
