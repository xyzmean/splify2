...
const FAIL_K = 3
/** Как долго верить запомненным версиям выпусков — ПОЛЧАСА, столько же, сколько помнит бэкенд.
 *
 *  Было шесть часов, и цена этого оказалась выше ожидаемой: выпуск вышел, роутер о нём уже знал
 *  (`ubus call splify2 steer_versions` отдавал новую версию первой), а страница показывала старый
 *  список — и перезагрузка не помогала, потому что проверка «свежо ли» стоит ДО запроса. Нажать
 *  «проверить» было негде, оставалось ждать. Полчаса — это не больше четырёх походов в интернет
 *  за час на роутер (по два на каждый из двух перечней, steer и splify2; чаще не выйдет и при
 *  нескольких открытых страницах — их отсекает память бэкенда), и они уходят через 1,2 с после
 *  отрисовки, ничего не задерживая. */
const VERSIONS_TTL_MS = 30 * 60 * 1000
...
export function useLive(): Live {
    ...
    useEffect(() => {
        ...
        const finish = (
             ...
             *
             * И только когда состояние ПРИШЛО: прежний круг (loadLegacy) зовёт finish и при
             * отказе status, и снятие признака здесь затирало `stale`, выставленный failed() на
             * первом промолчавшем круге, — снимок из памяти браузера показывался живым, с
             * зелёной точкой. */
            if (st) setStale(cachedNow)
