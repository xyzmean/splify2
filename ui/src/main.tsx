...
function mountWhenReady() {
  ...
  if (!el) return
  /* УЖЕ СМОНТИРОВАННОЕ НЕ ТРОГАЕМ, и это не перестраховка.
   *
   * Ленивый кусок раздела импортирует общие модули ИЗ ТОЧКИ ВХОДА (rollup держит их там,
   * потому что их читает и она), а ссылка внутри куска несёт свой `?v=`. Стоит этому адресу
   * разойтись с тем, которым бандл загрузил загрузчик, — и браузер заводит ВТОРОЙ экземпляр
   * модуля: он исполняется при первом же открытии раздела, доходит до этой строки, сносит
   * живой корень и монтирует приложение заново. Снаружи это выглядит так, как и было поймано
   * на роутере: раздел открывается, а через мгновение на его месте снова главная.
   *
   * Смену сборки это не ломает: новый модуль монтирует загрузчик, вызывая __splifyMount
   * по факту загрузки (view/splify2/home.js), — а не эта строка. */
  if (window.__splifyRoot) { mountedByMe = true; return }
  mountedByMe = true
  mount(el)
}
