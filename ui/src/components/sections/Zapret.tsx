            ...
            <Card>
                ...
                <CardContent className="space-y-3 text-sm">
                    ...
                    {/* Подбор отложен или упал — это ответ, и он важнее приговора по старым
                        числам: кнопку нажали, а ничего не произошло. Подбор идёт только на
                        свободном роутере (загрузка ниже 1.00, наружу меньше 300 кбит/с) — и
                        для расписания, и по кнопке: иначе замер мерил бы чужой трафик, а
                        человек получал бы просадку. Причину называет сам подбор. */}
                    {auto && !auto.running && auto.state === 'skipped' && (
                        <div className="rounded-lg bg-warning/10 px-3 py-2 text-xs text-warning-fg">
                            {t('Подбор отложен')}{auto.state_note ? `: ${auto.state_note}` : ''}.{' '}
                            {t('Он идёт только когда роутер свободен — попробуйте, когда через него не идёт трафик.')}
                        </div>
                    )}
