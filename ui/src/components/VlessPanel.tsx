...
export default function VlessPanel({ name, output, onChange, saved }: Props) {
    ...
    return (
        <div className="space-y-2 rounded-md border border-border p-2">
            ...
            {/* Сказанное панелью про устройство. Панели, привязывающие подписку к
              * устройствам, отвечают клиенту без идентификатора не отказом, а ЗАГЛУШКОЙ:
              * HTTP 200 и пара законных ссылок vless:// в никуда, где сообщение спрятано в
              * имя узла. Без этой строки отказ выглядит как «подписка скачалась, узлы не
            ...
            {/* Идентификатор роутера — рядом с полем подписки, потому что нужен он ровно
              * здесь: в панели или боте поставщика человек заводит устройство и освобождает
              * слот, а найти среди телефонов свою коробку можно только по этой строке.
              * Показывается только когда источник — подписка: вставленным руками ссылкам
